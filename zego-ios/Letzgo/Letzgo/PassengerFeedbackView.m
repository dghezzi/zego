...
@implementation PassengerFeedbackView
...
-(void)baseInit {
    ...
    [img setImageWithURL:[NSURL URLWithString:request.driver.userimg] placeholderImage:[UIImage imageNamed:@"userplaceholder"]];
    [self addSubview:img];
    
    UILabel* name = [[UILabel alloc] initWithFrame:CGRectMake(110, 3, w-110, 37)];
    name.font = [self boldFontWithSize:16];
    name.text = [NSString stringWithFormat:NSLocalizedString(@"Dai un voto a %@ per questo passaggio",nil),request.driver.name];
...
-(void)feedback {
    ...
    f.sender = request.pid;
    f.rid = request.rid;
    f.uid = request.did;
    f.rating = starRatingView.value;
    [self.delegate passengerFeedbackViewSubmitFeedback:f];
}
